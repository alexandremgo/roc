...
bool parse_sdp(const char* str, SessionDescription& result) {
    ...
    %%{
        ...
        origin_addrtype = ("IP4" %{ result.set_origin_addrtype(IP4); } 
                            | "IP6" %{ result.set_origin_addrtype(IP6); }
                        );
        # TODO start_p ici
        origin_unicast_address = non_ws_string >start_token %{  end_p_origin_addr = p; } 

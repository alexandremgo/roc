...
int main(int argc, char** argv) {
    ...
    if (!args.source_given && !args.repair_given) {
        roc_log(LogError, "at least --source or --repair port should be specified");
        return 1;
    }
    ...
    if (args.source_given) {
        ...
        if (args.miface_given) {
            if (!pipeline::set_miface_from_string(args.miface_arg, port.address)) {
